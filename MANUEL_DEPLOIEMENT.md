...
5. [Déploiement A — SQLite (mono-poste bureau)](#5-déploiement-a--sqlite-mono-poste-bureau)
6. [Déploiement B — PostgreSQL centralisé (serveur + clients navigateur)](#6-déploiement-b--postgresql-centralisé-serveur--clients-navigateur)
...
- **Un seul processus** écrit la base (contrainte SQLite + APScheduler). Ne lancez
  jamais plusieurs instances qui relèvent la même boîte (voir [§15](#15-invariants-à-respecter)).
...
| Élément | Détail |
|---|---|
| OS | Windows 10 / 11 (la cible de production ; le chiffrement DPAPI y est natif) |
| Python | 3.11 ou supérieur (si déploiement depuis les sources / venv) |
| PostgreSQL | **17** (seulement pour le déploiement B) |
...
> un `.exe` déjà construit, **reconstruisez-le** après cet ajout (voir [§12](#12-construction-de-lexécutable-windows)).

---

## 5. Déploiement A — SQLite (mono-poste bureau)

Le mode le plus simple : tout tient sur une machine, base SQLite locale.
...
## 6. Déploiement B — PostgreSQL centralisé (serveur + clients navigateur)

C'est le cas d'un serveur unique avec base PostgreSQL locale sur ce même serveur.
**PostgreSQL peut rester en `localhost`** : seule l'application du serveur y accède,
il est donc inutile (et déconseillé) d'exposer le port 5432 au réseau.
...
- **Sans `CV_AGENT_SECRET`**, les secrets chiffrés en DPAPI **ne suivent pas** si vous
  ...
  ressaisir. C'est pourquoi un déploiement PostgreSQL centralisé doit poser
  `CV_AGENT_SECRET` (même valeur partout où l'app tourne).
...
- ⚠️ **Ne versionnez jamais** `state.db`, `session.secret`, `cv_pdfs/`, `logs/` — ils
...
|---|---|
| **Ollama** (local, recommandé) | Installer Ollama, télécharger le modèle (`ollama pull qwen2.5:14b`), vérifier qu'il écoute sur `http://localhost:11434`. 100 % local, aucune donnée envoyée. |
...
**`cv_pdfs/`** (les PDF des candidats).

### Où vivent les données

| Donnée | Emplacement (dev / sources) | Emplacement (exe figé) |
|---|---|---|
| Base SQLite | `state.db` (dossier projet) | `%LOCALAPPDATA%\CV-Agent-Pro\state.db` |
| PDF des CV | `cv_pdfs/` | `%LOCALAPPDATA%\CV-Agent-Pro\cv_pdfs\` |
| Journaux | `logs\agent.log` | `%LOCALAPPDATA%\CV-Agent-Pro\logs\agent.log` |
| Secret de session | `session.secret` | `%LOCALAPPDATA%\CV-Agent-Pro\session.secret` |

> Toute écriture passe par `app_paths.data_path()` : jamais de chemin relatif brut,
> pour rester valide une fois l'application figée en `.exe`.
...
- **Un seul worker uvicorn.** APScheduler et l'écriture base supposent un process
  unique (`start_web.bat` passe `--workers 1`). Ne montez pas à plusieurs workers.
- **Une seule instance qui relève l'IMAP.** Ne faites pas tourner deux serveurs
  applicatifs qui relèvent la **même** boîte : ils dédoublonneraient mal et se
  disputeraient l'écriture. En architecture serveur unique, ce risque n'existe pas.
  Si vous multipliez les instances (déconseillé), désactivez le planificateur sur
...
- **Journaux du pipeline** : `logs\agent.log` (relève, classification, extraction).
  Pour la tâche planifiée, la sortie de démarrage va dans `logs\web_startup.log`.
...
| Les postes clients n'accèdent pas au serveur | Règle pare-feu 6060 absente / trop restrictive, ou l'app écoute sur `127.0.0.1` au lieu de `0.0.0.0`. Utilisez `run_web.bat` / `start_web.bat`. |
...
| Secrets « vides » après copie de `state.db` | Blobs DPAPI (`enc:v1`) liés à l'ancienne machine. Sur un déploiement partagé, posez `CV_AGENT_SECRET` (chiffrement portable `enc:v2`) et ressaisissez les secrets une fois. |
...
| `CV_AGENT_SECRET` | (1) stabilise le cookie de session entre redémarrages ; (2) active le chiffrement portable des secrets (`enc:v2`). Requis si plusieurs instances partagent une base PostgreSQL. | `<64 hex générés>` (voir §6.2) |
...
### 18.3 Checklist de déploiement B (PostgreSQL centralisé)
