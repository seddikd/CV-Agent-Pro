...
**PostgreSQL is the single source of truth.** `config.yaml` is only a first-run *seed* consumed by `bootstrap.py`; at runtime all configuration lives in the `settings` table and is edited via the admin UI, not files. `web_db.settings_to_config()` flattens the DB settings into the nested `cfg` dict every pipeline module consumes — when adding a config key, wire it in `DEFAULT_SETTINGS`, the `MAIL_FIELDS`/`LLM_FIELDS` form lists in `webapp.py`, and `settings_to_config()` (e.g. `imap.security` = SSL/STARTTLS/None, wired in all four places). If the key should also be seedable from `config.yaml`, add it to the `mapping` dict in `bootstrap.py` too — a key absent there is silently ignored at first run. Booleans are stored as the **strings** `"true"`/`"false"` (compared with `== "true"`); `bootstrap.py` lowercases YAML booleans for that reason. Form fields of kind `bool` render as an Activé/Désactivé `<select>` in `admin_settings.html`.

**Pipeline flow** (`web_pipeline.run_pipeline` orchestrates one cycle):
`mail_fetcher` (IMAP) → `pdf_extractor` (PDF/DOCX → text) → `llm_classifier` (is this a CV?) → `llm_extractor` (structured fields) → `web_db.insert_candidate`, then optionally `mail_fetcher.move_processed_messages` (see « Mailbox writes »). It is triggered three ways, all sharing the same function: the in-process **APScheduler** job, the "Run now" admin button, and `main.py`.
...
- **Mailbox writes are opt-in and never touch unprocessed mail.** Fetching is strictly read-only (`mark_seen=False`, no flags, no move); dedup lives in `processed_emails`, not in mailbox state. Two *separate* "range les traités" paths write back: **IMAP** — `mail_fetcher.move_processed_messages()`, keyed by **UID** (same folder that produced the UIDs, so the match is unambiguous), run automatically at end of cycle when `imap.move_processed` is on and the run was not cancelled (best effort — a failure is logged, never fails the run), or on demand via `POST /admin/settings/move-imap`; **PST/OST** — `outlook_fetcher.move_cv_messages()`, keyed by **Message-ID** (the only key that survives crossing from a file copy to the live Outlook store), needs win32com + Outlook. Both move **only** mails already in `processed_emails`, and both split CV vs non-CV into two folders. Keep that restriction: moving a mail the pipeline never analysed would lose it from the polled folder without any trace.
...
- **Colours live in the CSS tokens, never inline.** The Rayanox brand palette is defined once in `static/style.css` (`--primary #295894`, `--primary-strong #233873`, `--primary-soft`, `--accent #1e8dcc`; the dark theme *inverts* the scale). Roles were assigned from measured WCAG contrast, not by taste: `--primary` carries links and 10.5 px text so it must stay ≥ 4.5:1 — which is why the vivid `#1e8dcc` is reserved for accents and logo gradients and must **not** be used as small text. Style new UI with the tokens (`var(--primary)`, `color-mix(...)`); a hardcoded hex silently escapes the dark theme, which is exactly how the dark sidebar-active regression happened. The amber/pastel accents (admin badge, « à contacter » status, KPI tiles) are **semantic** signals — do not fold them into the blue palette.
