...
- **Secrets are encrypted at rest.** `imap.password`, `smtp.password`, `openrouter.api_key` (the `SECRET_SETTING_KEYS` set in `web_db.py`) are encrypted via `secret_store.py` (Windows DPAPI, machine scope) transparently inside `web_db.set_settings`/`get_all_settings` — callers always see plaintext in memory. Stored form is `enc:v1:…`; unprefixed values are legacy plaintext (auto-migrated at startup). DPAPI blobs are machine-bound, so a copied `state.db` cannot be decrypted elsewhere. **Never put real credentials in `config.yaml`** — it is bundled into the distributed exe.
