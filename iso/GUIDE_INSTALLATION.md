...
poste Windows 10 ou 11 : l'application, un installateur automatique et les
prérequis (PostgreSQL). Les manuels sont dans le dossier `Manuels\`.
...
├─ Application\CV-Agent\             L'application (CV-Agent.exe + dépendances)
├─ Prerequis\
...
| Internet | Requis **pendant l'installation** (téléchargement de PostgreSQL / Ollama) |
| winget | « App Installer », présent par défaut sur Windows 10/11 à jour |
...
> **PostgreSQL est requis** : l'installateur l'installe automatiquement. Il n'y a
> plus de base fichier (SQLite) — toutes les données vivent dans PostgreSQL.
...
2. L'installateur enchaîne **6 étapes** :
   1. **PostgreSQL 17** — installation silencieuse (mot de passe superutilisateur
      généré et enregistré dans `…\postgres-superuser-password.txt`).
   2. **Ollama** (si `-InstallOllama`) — moteur LLM local.
...
## 5. Alternative : déploiement Docker
...
- **Application** : remplacez le contenu de `C:\CV-Agent-Pro` par la nouvelle
  version (`Application\CV-Agent\`). Les données restent dans PostgreSQL (intactes).
...
|---|---|
| « winget est introuvable » | Installez « App Installer » depuis le Microsoft Store, puis relancez. |
